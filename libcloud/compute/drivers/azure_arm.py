...
class AzureNetworkSecurityGroup(object):
    ...
    def __init__(self, id, name, location, tags, extra):
        self.id = id
        self.name = name
        self.location = location
        self.tags = tags
...
class AzureNodeDriver(NodeDriver):
    ...
    def ex_list_security_groups(self):
        ...
        action = "/subscriptions/%s/providers/Microsoft.Network/networkSecurityGroups" % (self.subscription_id)
        r = self.connection.request(action,
                                    params={"api-version": "2015-06-15"})
        return [
            AzureNetworkSecurityGroup(
                ...
                tags=sg.get('tags', None),
                extra=sg['properties']
            ) for sg in r.object['value']
        ]

    def ex_delete_security_group(self, resource_group, name):
        """
        Delete a network security group within a resource group by name
        """

        action = "/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Network/networkSecurityGroups/%s" % \
        ...
        if r.status == 202:
            ...
            raise ValueError('Security group with supplied values does not exist')
        else:
            return False

    def ex_delete_security_group_rule(self, resource_group, group_name, rule_name):
        """
        Delete a network security rule within a network security group
        """

        action = "/subscriptions/%s/resourceGroups/%s/providers/Microsoft.Network/networkSecurityGroups/%s/securityRules/%s" % \
                 (self.subscription_id, resource_group, group_name, rule_name)
        r = self.connection.request(action,
                                    method='DELETE',
                                    params={"api-version": "2015-06-15"})
        ...
        if r.status == 202:
            ...
            raise ValueError('Security rule does not exist')
        else:
            return False
